...
public abstract class CodecDataProvider<T> implements DataProvider {
    ...
    protected void register(ResourceKey<T> key, T object) {
        if(entries.containsKey(key))
            throw new IllegalStateException("Tried to register duplicate data at " + key.identifier() + " for data type " + name + "!");
        entries.put(key.identifier(), object);
    }

    protected void register(Identifier key, T object) {
        if(entries.containsKey(key))
