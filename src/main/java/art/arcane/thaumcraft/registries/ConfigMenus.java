...
public final class ConfigMenus {
    ...
    private static <T extends AbstractContainerMenu> Supplier<MenuType<T>> register(Identifier id, MenuType.MenuSupplier<T> factory) {
        return REGISTRY.register(id.getPath(), () -> new MenuType<>(factory, FeatureFlags.DEFAULT_FLAGS));
    }

    private static <T extends AbstractContainerMenu> Supplier<MenuType<T>> registerFactory(Identifier id, IContainerFactory<T> factory) {
